...
const LineSidebar = ({
  ...
  /** Index under the cursor, or -1. Replaces the old proximity targets. */
  ...
  const runFrame = useCallback((now: number) => {
    ...
    for (let i = 0; i < items.length; i++) {
      const el = items[i];
      if (!el) continue;
      // Exactly one item is ever lit: whichever is hovered, or — when nothing
      // is — the active section. No distance falloff, so neighbours never
      // light up alongside it.
      const hovered = hoveredRef.current;
      const target = (hovered >= 0 ? hovered === i : activeRef.current === i) ? 1 : 0;
